...
get '/:bank' do
  ...
  case params[:bank]
  when "agd"
    JSON.pretty_generate(Currencies.new(Nokogiri::HTML(open(sources[:agd])),"AGD").to_hash)
  when "cb"
    JSON.pretty_generate(Currencies.new(Nokogiri::HTML(open(sources[:cb])),"CB").to_hash)
  when "kbz"
